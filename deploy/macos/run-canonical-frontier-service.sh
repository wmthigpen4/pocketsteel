...
case "$STEEL_RAG_CANONICAL_FRONTIER_RUNTIME_DIR/" in
  "$STEEL_RAG_CANONICAL_FRONTIER_SERVICE_ROOT/"*) fail "runtime state must be outside the immutable service root" ;;
esac
...
STEEL_RAG_CANONICAL_FRONTIER_FROZEN_INDEX="$STEEL_RAG_CANONICAL_FRONTIER_SERVICE_ROOT/rag-evaluation/index/steel-forum-passages-v1-hybrid-v1"
STEEL_RAG_CANONICAL_FRONTIER_RUNTIME_INDEX="$STEEL_RAG_CANONICAL_FRONTIER_RUNTIME_DIR/index"
STEEL_RAG_CANONICAL_FRONTIER_RUNTIME_MARKER="$STEEL_RAG_CANONICAL_FRONTIER_RUNTIME_DIR/bundle-manifest.sha256"
STEEL_RAG_CANONICAL_FRONTIER_BUNDLE_SHA="$(shasum -a 256 "$STEEL_RAG_CANONICAL_FRONTIER_MANIFEST" | awk '{print $1}')"
if [[ -e "$STEEL_RAG_CANONICAL_FRONTIER_RUNTIME_DIR" ]]; then
  [[ -r "$STEEL_RAG_CANONICAL_FRONTIER_RUNTIME_MARKER" ]] || fail "runtime state exists without a verified marker"
  [[ "$(tr -d '[:space:]' < "$STEEL_RAG_CANONICAL_FRONTIER_RUNTIME_MARKER")" == "$STEEL_RAG_CANONICAL_FRONTIER_BUNDLE_SHA" ]] \
    || fail "runtime state belongs to a different bundle"
  [[ -d "$STEEL_RAG_CANONICAL_FRONTIER_RUNTIME_INDEX/vector" ]] || fail "runtime vector state is incomplete"
else
  STEEL_RAG_CANONICAL_FRONTIER_RUNTIME_PARENT="$(dirname "$STEEL_RAG_CANONICAL_FRONTIER_RUNTIME_DIR")"
  mkdir -p "$STEEL_RAG_CANONICAL_FRONTIER_RUNTIME_PARENT"
  STEEL_RAG_CANONICAL_FRONTIER_RUNTIME_TEMP="$(mktemp -d "$STEEL_RAG_CANONICAL_FRONTIER_RUNTIME_PARENT/.canonical-frontier-runtime.XXXXXX")"
  mkdir -p "$STEEL_RAG_CANONICAL_FRONTIER_RUNTIME_TEMP/index"
  ln -s "$STEEL_RAG_CANONICAL_FRONTIER_FROZEN_INDEX/manifest.json" "$STEEL_RAG_CANONICAL_FRONTIER_RUNTIME_TEMP/index/manifest.json"
  ln -s "$STEEL_RAG_CANONICAL_FRONTIER_FROZEN_INDEX/vector-embedding-progress.json" "$STEEL_RAG_CANONICAL_FRONTIER_RUNTIME_TEMP/index/vector-embedding-progress.json"
  ln -s "$STEEL_RAG_CANONICAL_FRONTIER_FROZEN_INDEX/lexical.sqlite3" "$STEEL_RAG_CANONICAL_FRONTIER_RUNTIME_TEMP/index/lexical.sqlite3"
  cp -cR "$STEEL_RAG_CANONICAL_FRONTIER_FROZEN_INDEX/vector" "$STEEL_RAG_CANONICAL_FRONTIER_RUNTIME_TEMP/index/vector"
  chmod -R u+rwX "$STEEL_RAG_CANONICAL_FRONTIER_RUNTIME_TEMP/index/vector"
  printf '%s\n' "$STEEL_RAG_CANONICAL_FRONTIER_BUNDLE_SHA" > "$STEEL_RAG_CANONICAL_FRONTIER_RUNTIME_TEMP/bundle-manifest.sha256"
  mv "$STEEL_RAG_CANONICAL_FRONTIER_RUNTIME_TEMP" "$STEEL_RAG_CANONICAL_FRONTIER_RUNTIME_DIR"
fi
export STEEL_RAG_FRONTIER_RUNTIME_INDEX_ROOT="$STEEL_RAG_CANONICAL_FRONTIER_RUNTIME_INDEX"
