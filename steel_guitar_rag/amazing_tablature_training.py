...
from steel_guitar_rag.amazing_tablature_input_parity import structured_input_parity_report
...
from steel_guitar_rag.e9_copedents import E9CopedentProfile, e9_copedent_profile_digest, get_e9_copedent_profile
from steel_guitar_rag.melody_decision_rules import normalize_style_family
from steel_guitar_rag.melody_ranker import feature_vector, score_candidate, train_pairwise_ranker
...
class AmazingTablatureTrainingStore:
    ...
    def _combined_line_decisions(
        ...
        from steel_guitar_rag.amazing_tablature_decisions import derive_decision_annotations
    ...
    def shadow_test_discovery(self, model_id: str, *, max_review_lines: int = 12) -> dict[str, Any]:
        ...
        from steel_guitar_rag.amazing_tablature_decisions import derive_decision_annotations
        from steel_guitar_rag.amazing_tablature_extraction import _score_audit_equivalence_gates
    ...
    def adjudicate_validation_machine_disagreements(
        ...
        *,
        ...
        """Measure human-accepted alternatives without training on validation."""

        if not re.fullmatch(r"[0-9a-f]{64}", score_report_digest):
            raise TrainingWorkflowError(
                "Validation adjudication needs an exact score-report digest."
            )
        if not re.fullmatch(
            r"validation-disagreement-submission-[0-9a-f]{20}",
            submission_id,
            ...
            raise TrainingWorkflowError(
                "Validation adjudication needs an exact submission ID."
            )
        registry = self._registry()
        model_meta = (registry.get("models") or {}).get(model_id)
        if not isinstance(model_meta, Mapping):
        ...
        if batch_id not in authoritative_ids:
            raise TrainingWorkflowError(
                "Validation adjudication requires an authoritative cohort."
            )
        model_path = self.root / str(model_meta.get("artifact") or "")
        if not model_path.exists():
            raise TrainingWorkflowError("The challenger artifact is missing.")
        model_sha256 = _sha256_bytes(model_path.read_bytes())
        if model_sha256 != str(model_meta.get("artifactSha256") or ""):
            raise TrainingWorkflowError(
                "The challenger artifact changed before adjudication."
            )
        ...
        if not score_path.exists():
            raise TrainingWorkflowError(
                "The exact machine-consensus score report is missing."
            )
        ...
        disagreement_digest = str(
            score_report.get("disagreementDigest") or ""
        )
        if (
            ...
            or not re.fullmatch(r"[0-9a-f]{64}", disagreement_digest)
        ):
            raise TrainingWorkflowError(
                "Machine-consensus score lineage or no-training contract failed."
            )
        disagreement_path = (
            evaluation_dir
            / f"machine-consensus-disagreements-{disagreement_digest}.json"
        ...
        if not disagreement_path.exists():
            raise TrainingWorkflowError(
                "The exact disagreement report is missing."
            )
        ...
        if (
            ...
            or disagreement_report.get("modelArtifactSha256")
            != model_sha256
            or disagreement_report.get("reportDigest")
            != disagreement_digest
            ...
            or int(disagreement_report.get("disagreementCount") or 0)
            != len(disagreements)
        ):
            raise TrainingWorkflowError(
                "Validation disagreement lineage or no-training contract failed."
            )
        ...
        metadata_path = review_dir / "submissions" / f"{submission_id}.json"
        submission_path = review_dir / "submissions" / f"{submission_id}.jsonl"
        if not metadata_path.exists() or not submission_path.exists():
            raise TrainingWorkflowError(
                "The exact validation adjudication receipt is missing."
            )
        metadata = _read_json(metadata_path)
        packet_digest = str(metadata.get("packetDigest") or "")
        packet_path = review_dir / f"packet-{packet_digest}.json"
        if not packet_path.exists():
            raise TrainingWorkflowError(
                "The adjudicated validation packet is missing."
            )
        packet = _read_json(packet_path)
        packet_core = {
            key: value for key, value in packet.items() if key != "packetDigest"
        ...
        if (
            ...
            or packet.get("disagreementReportDigest")
            != disagreement_digest
            ...
            raise TrainingWorkflowError(
                "Validation adjudication packet lineage or privacy failed."
            )
        expected_ids = {
            str(item.get("decisionId") or "")
            for system in packet.get("systems") or ()
        ...
        cohort_disagreement_ids = {
            str(item.get("decisionId") or "")
            for item in disagreements
            if str(item.get("batchId") or "") == batch_id
        }
        if expected_ids != cohort_disagreement_ids:
            raise TrainingWorkflowError(
                "Validation adjudication does not cover the exact cohort disagreements."
            )
        ...
        if (
            ...
            or metadata.get("disagreementReportDigest")
            != disagreement_digest
            or int(metadata.get("reviewCount") or 0) != len(rows)
            ...
            raise TrainingWorkflowError(
                "Validation adjudication receipt lineage or privacy failed."
            )
        ...
        for row in rows:
            ...
            status = str(row.get("status") or "")
            if (
                decision_id not in expected_ids
                or decision_id in statuses
                or status
                not in {
                    "source_preferred",
                    "challenger_valid",
                ...
                raise TrainingWorkflowError(
                    "Validation adjudication contains an unknown decision."
                )
            statuses[decision_id] = status
            status_counts[status] = status_counts.get(status, 0) + 1
        if set(statuses) != expected_ids:
            raise TrainingWorkflowError(
                "Validation adjudication is incomplete."
            )

        accepted_statuses = {"challenger_valid", "both_valid"}
        human_accepted = sum(
            status in accepted_statuses for status in statuses.values()
        )
        unresolved = sum(
            status == "feedback" for status in statuses.values()
        )
        strict_metrics = deepcopy(score_report.get("metrics") or {})
        strict_correct = int(
            strict_metrics.get("topChoiceCorrectCount") or 0
        )
        decision_count = int(strict_metrics.get("decisionCount") or 0)
        accepted_correct = strict_correct + human_accepted
        accepted_metrics = {
            **strict_metrics,
            "strictSourceTopChoiceCorrectCount": strict_correct,
            "strictSourceTopChoiceAccuracy": (
                strict_correct / decision_count if decision_count else 0.0
            ),
            "humanAcceptedAlternativeCount": human_accepted,
            "humanRejectedAlternativeCount": status_counts.get(
                "source_preferred",
                0,
            ),
            "humanUnresolvedAlternativeCount": unresolved,
            "acceptedTopChoiceCorrectCount": accepted_correct,
            "acceptedTopChoiceAccuracy": (
                accepted_correct / decision_count if decision_count else 0.0
            ),
        }
        cohort_metrics = deepcopy(score_report.get("cohortMetrics") or {})
        selected_cohort = cohort_metrics.get(batch_id)
        if not isinstance(selected_cohort, Mapping):
            raise TrainingWorkflowError(
                "Validation adjudication cohort metrics are missing."
            )
        selected_count = int(selected_cohort.get("decisionCount") or 0)
        selected_strict = int(
            selected_cohort.get("topChoiceCorrectCount") or 0
        )
        cohort_metrics[batch_id] = {
            **selected_cohort,
            "strictSourceTopChoiceCorrectCount": selected_strict,
            "strictSourceTopChoiceAccuracy": (
                selected_strict / selected_count if selected_count else 0.0
            ),
            "humanAcceptedAlternativeCount": human_accepted,
            "humanUnresolvedAlternativeCount": unresolved,
            "acceptedTopChoiceCorrectCount": selected_strict
            + human_accepted,
            "acceptedTopChoiceAccuracy": (
                (selected_strict + human_accepted) / selected_count
                if selected_count
                else 0.0
            ),
        }
        for other_batch_id, metrics in cohort_metrics.items():
            if other_batch_id == batch_id or not isinstance(metrics, Mapping):
                continue
            count = int(metrics.get("decisionCount") or 0)
            correct = int(metrics.get("topChoiceCorrectCount") or 0)
            cohort_metrics[other_batch_id] = {
                **metrics,
                "strictSourceTopChoiceCorrectCount": correct,
                "strictSourceTopChoiceAccuracy": (
                    correct / count if count else 0.0
                ),
                "humanAcceptedAlternativeCount": 0,
                "humanUnresolvedAlternativeCount": 0,
                "acceptedTopChoiceCorrectCount": correct,
                "acceptedTopChoiceAccuracy": (
                    correct / count if count else 0.0
                ),
            }
        ...
        preference_thresholds_passed = bool(
            ...
            and unresolved == 0
            and accepted_metrics["acceptedTopChoiceAccuracy"]
            > VALIDATION_OVERALL_PREFERENCE_FLOOR
            and float(accepted_metrics.get("topThreeCoverage") or 0.0)
            >= VALIDATION_TOP_THREE_COVERAGE_FLOOR
            and float(
                accepted_metrics.get("approvedSourceMechanicalAccuracy")
                or 0.0
            )
            == 1.0
            and float(
                accepted_metrics.get("predictedTopMechanicalAccuracy")
                or 0.0
            )
            == 1.0
            and all(
        ...
        report_core = {
            ...
            "evaluatedAt": _utc_now(),
            "sourceScoreReportDigest": score_report_digest,
            "sourceDisagreementReportDigest": disagreement_digest,
            "batchId": batch_id,
            "submissionId": submission_id,
            "submissionDigest": submission_digest,
            ...
            "gate": {
                "adjudicatedPreferenceThresholdsPassed": (
                    preference_thresholds_passed
                ),
                "canonicalGatePassed": False,
                "rulesFreezeAllowed": False,
                "privateRuntimeEnableAllowed": False,
                "reasons": [
                    ...
                    *(
                        ...
                        else ["adjudicated_preference_thresholds_not_met"]
                    ),
                ],
            },
            "thresholds": deepcopy(score_report.get("thresholds") or {}),
            "lineage": {
        ...
        report_path = (
            evaluation_dir
            / f"machine-consensus-adjudication-{report_digest}.json"
        )
        _write_json(report_path, report)
        _make_private(report_path)
        return {
    ...
    def score_corrected_canonical_validation(
        self,
        model_id: str,
        *,
        ...
        Validation corrections are never imported into discovery or training.
        Previously reviewed preference verdicts are carried only when the full
        disagreement signature is byte-for-byte equivalent.
        """
        ...
        for label, value in {
            ...
            "source adjudication": source_adjudication_digest,
        }.items():
            if not re.fullmatch(r"[0-9a-f]{64}", value):
                raise TrainingWorkflowError(
                    f"Corrected canonical validation needs an exact {label} digest."
                )
        registry = self._registry()
        model_meta = (registry.get("models") or {}).get(model_id)
        if not isinstance(model_meta, Mapping):
        ...
        if (
            ...
            raise TrainingWorkflowError(
                "Corrected canonical validation requires the exact "
                "complete-discovery challenger."
            )
        model_path = self.root / str(model_meta.get("artifact") or "")
        model_sha256 = str(model_meta.get("artifactSha256") or "")
        if (
        ...
        if not correction_path.exists():
            raise TrainingWorkflowError(
                "The corrected canonical validation report is missing."
            )
        ...
        correction_lineage = correction.get("lineage") or {}
        extraction_code_path = (
            self.repo_root / "steel_guitar_rag" / "amazing_tablature_extraction.py"
        )
        receipts = [
            deepcopy(value)
        ...
        if (
            ...
            != correction_report_digest
            or correction.get("allCorrectedLinesMechanicallyValid")
            is not True
            or int(correction.get("confirmationRequiredLineCount") or 0)
            != 0
            or int(correction.get("lineCount") or 0) != len(receipts)
            or len(receipts) != 9
            or any(
                bool(receipt.get("requiresConfirmation"))
                or bool(receipt.get("requiresScoreConfirmation"))
                or receipt.get("trainingEligible") is not False
            ...
            != tuple(sorted(authoritative_ids))
            ...
            raise TrainingWorkflowError(
                "Corrected canonical truth lineage or privacy contract failed."
            )
        ...
        if (
            not re.fullmatch(r"[0-9a-f]{64}", source_packet_digest)
            or not source_packet_path.exists()
        ):
            raise TrainingWorkflowError(
                "Corrected canonical truth lost its source packet."
            )
        source_packet = _read_json(source_packet_path)
        source_packet_core = {
            key: value
        ...
        if (
            ...
            or set(packet_lines)
            != {
                str(receipt.get("lineId") or "")
                for receipt in receipts
            }
        ):
            raise TrainingWorkflowError(
                "Corrected canonical source packet lineage failed."
            )

        candidate_indexes: dict[str, dict[str, Mapping[str, Any]]] = {}
        ranking_records: list[dict[str, Any]] = []
        truth_line_receipts: list[dict[str, Any]] = []
        for receipt in receipts:
            batch_id = str(receipt.get("batchId") or "")
            input_id = str(receipt.get("inputId") or "")
            tab_system_id = str(receipt.get("tabSystemId") or "")
            line_id = str(receipt.get("lineId") or "")
            evidence_mode = str(receipt.get("evidenceMode") or "")
            if evidence_mode not in {"score_supported", "tab_only"}:
                raise TrainingWorkflowError(
                    "Corrected canonical truth has an invalid evidence mode."
                )
            batch_dir = self._batch_dir(batch_id)
            ...
            if not page_path.exists():
                raise TrainingWorkflowError(
                    "A corrected canonical source page is missing."
                )
            page = _read_json(page_path)
            manifest, _state = self._batch(batch_id)
            profile = _profile_for_source(
                str(manifest["sourceCopedentId"])
            )
            if (
                profile.revision
                != int(manifest["sourceCopedentRevision"])
                or _profile_digest(profile)
                != str(manifest["sourceCopedentDigest"])
            ):
                raise TrainingWorkflowError(
                    "A corrected canonical copedent changed."
                )
            ...
            if corrected_relative:
                corrected_path = (self.root / corrected_relative).resolve()
                try:
                    corrected_path.relative_to(correction_dir.resolve())
                except ValueError as exc:
                    raise TrainingWorkflowError(
                        "A corrected canonical line escaped its private root."
                    ) from exc
                if not corrected_path.exists():
                    raise TrainingWorkflowError(
                        "A corrected canonical line is missing."
                    )
                ...
                if (
                    corrected.get("correctedLineDigest") != truth_digest
                    or _sha256_json(corrected_core) != truth_digest
                    or corrected.get("batchId") != batch_id
                    or corrected.get("inputId") != input_id
                    or corrected.get("tabSystemId") != tab_system_id
                    or corrected.get("mechanicallyValid") is not True
                    or corrected.get("validationGroundTruthMayTrain")
                    is not False
                    or corrected.get("sealedTestAccessed") is not False
                ):
                    raise TrainingWorkflowError(
                        "A corrected canonical line changed after confirmation."
                    )
                events = deepcopy(corrected.get("events") or [])
                truth_origin = "human_confirmed_correction"
            else:
                candidate_digest = str(
                    receipt.get("candidateDigest") or ""
                )
                if batch_id not in candidate_indexes:
                    consensus_path = (
                        validation_root
                        / "review"
                        / "automation"
                    ...
                    if not consensus_path.exists():
                        raise TrainingWorkflowError(
                            "A corrected canonical source consensus is missing."
                        )
                    consensus = _read_json(consensus_path)
                    candidate_indexes[batch_id] = {
                ...
                source_line = candidate_indexes[batch_id].get(
                    candidate_digest
                )
                if source_line is None:
                    raise TrainingWorkflowError(
                        "A confirmed original canonical line lost its candidate."
                    )
                candidate_path = (
                    validation_root
                    / str(source_line.get("candidatePath") or "")
                ).resolve()
                if not candidate_path.exists():
                    raise TrainingWorkflowError(
                        "A confirmed original candidate is missing."
                    )
                candidate = _read_json(candidate_path)
                candidate_core = {
                    key: value
                ...
                if (
                    ...
                    or _sha256_json(candidate_core) != candidate_digest
                    or candidate.get("batchId") != batch_id
                    or candidate.get("inputId") != input_id
                    or candidate.get("tabSystemId") != tab_system_id
                    or candidate.get("humanTruthUsed") is not False
                    or candidate.get("validationMayTrain") is not False
                    or candidate.get("sealedTestAccessed") is not False
                ):
                    raise TrainingWorkflowError(
                        "A confirmed original candidate lineage failed."
                    )
                events = deepcopy(candidate.get("events") or [])
                truth_origin = "human_confirmed_original"

            if (
                not events
                or validation_contact_execution_digest(events)
                != str(receipt.get("executionDigest") or "")
            ):
                raise TrainingWorkflowError(
                    "A corrected canonical execution digest changed."
                )
            for event in events:
                event["derivedMovementEligibility"] = {
                    "eligible": True,
                    "evidenceClass": "explicit_human_confirmation",
                    "reason": None,
                    "reviewState": "human_approved",
                }
            tab_system = next(
            ...
            if tab_system is None:
                raise TrainingWorkflowError(
                    "A corrected canonical tab system is missing."
                )
            tab_system["tabEvents"] = events
            derivation = deepcopy(page)
            derivation["tabSystems"] = [tab_system]
            # The ranker consumes normalized steel movement facts. Score
            # support remains a separately reported evidence stratum and does
            # not alter the candidate features being tested.
            derivation["scoreSystems"] = []
            derivation["eventAlignments"] = []
            event_ids = {
                str(value.get("tabEventId") or "") for value in events
            }
            derivation["movementSequences"] = [
                value
                for value in derivation.get("movementSequences") or ()
                if str(value.get("toTabEventId") or "") in event_ids
            ]
            decisions = derive_decision_annotations(
                derivation,
                profile,
            )
            if len(decisions) != max(0, len(events) - 1):
                raise TrainingWorkflowError(
                    "A confirmed corrected line did not yield one ranker "
                    "decision per movement transition."
                )
            for decision in decisions:
                decision["batchId"] = batch_id
                decision["validationEvidence"] = {
                    "mode": "human_corrected_canonical_truth",
                    "evidenceMode": evidence_mode,
                    "correctionReportDigest": correction_report_digest,
                    "truthLineDigest": truth_digest,
                    "truthOrigin": truth_origin,
                    "validationMayTrain": False,
                }
                decision["mechanicalValidation"] = (
                    _mechanical_validation(decision, profile)
                )
            ranking_records.extend(decisions)
            truth_line_receipts.append(
                {
                    "lineId": line_id,
                    "batchId": batch_id,
                    "inputId": input_id,
        ...
        if not source_adjudication_path.exists():
            raise TrainingWorkflowError(
                "The source preference adjudication is missing."
            )
        source_adjudication = _read_json(source_adjudication_path)
        source_adjudication_core = {
            key: value
        ...
        if (
            ...
            or source_adjudication.get("validationMayTrain") is not False
            or source_adjudication.get("sealedTestAccessed") is not False
            or not source_disagreement_path.exists()
        ):
            raise TrainingWorkflowError(
                "The source preference adjudication lineage failed."
            )
        source_disagreement = _read_json(source_disagreement_path)
        source_disagreement_core = {
            key: value
        ...
        for disagreement in source_disagreements or ():
            ...
            if status not in {
                ...
                raise TrainingWorkflowError(
                    "The source preference submission is incomplete."
                )
            signature_digest = _sha256_json(
                _validation_disagreement_equivalence_signature(
                    disagreement
                )
            )
            source_status_by_signature[signature_digest] = status
        ...
        for disagreement in disagreements:
            ...
            status = source_status_by_signature.get(signature_digest)
            if status is None:
                pending.append(disagreement)
            else:
                carried.append(
                    {
                        "decisionId": disagreement.get("decisionId"),
        ...
        def accepted_metrics(
            ...
            disagreements_by_id = {
                str(value.get("decisionId") or ""): value
                for value in self._validation_ranking_disagreements(
                    model,
                    records,
                )
            }
            accepted = sum(
                carried_statuses.get(decision_id)
                in {"challenger_valid", "both_valid"}
                for decision_id in disagreements_by_id
            )
            unresolved = sum(
                decision_id not in carried_statuses
                for decision_id in disagreements_by_id
            )
            strict = int(metrics["topChoiceCorrectCount"])
            count = int(metrics["decisionCount"])
            metrics.update(
                {
                    "strictSourceTopChoiceCorrectCount": strict,
                    "strictSourceTopChoiceAccuracy": (
                        strict / count if count else 0.0
                    ),
                    "humanAcceptedEquivalentAlternativeCount": accepted,
                    "humanUnresolvedAlternativeCount": unresolved,
                    "acceptedTopChoiceCorrectCount": strict + accepted,
                    "acceptedTopChoiceAccuracy": (
                        (strict + accepted) / count if count else 0.0
                    ),
                }
            )
            return metrics

        overall = accepted_metrics(ranking_records)
        cohort_metrics = {
            batch_id: {
                **accepted_metrics(
                    [
                        value
                        for value in ranking_records
                        if value.get("batchId") == batch_id
                    ]
                ),
                "evidenceSufficient": sum(
                    value.get("batchId") == batch_id
                    for value in ranking_records
                )
                >= VALIDATION_MIN_DECISIONS_PER_COHORT,
            }
        ...
        evidence_metrics = {}
        for evidence_mode in ("score_supported", "tab_only"):
            records = [
                value
                for value in ranking_records
                if (value.get("validationEvidence") or {}).get(
                    "evidenceMode"
                )
                == evidence_mode
            ]
            evidence_metrics[f"alignment:{evidence_mode}"] = {
                **accepted_metrics(records),
                "evidenceSufficient": len(records)
                >= VALIDATION_MIN_DECISIONS_PER_EVIDENCE_MODE,
            }
        gate_passed = bool(
            not pending
            and float(overall["acceptedTopChoiceAccuracy"])
            > VALIDATION_OVERALL_PREFERENCE_FLOOR
            and float(overall["topThreeCoverage"])
            >= VALIDATION_TOP_THREE_COVERAGE_FLOOR
            and float(overall["approvedSourceMechanicalAccuracy"]) == 1.0
            and float(overall["predictedTopMechanicalAccuracy"]) == 1.0
            and all(
                bool(value["evidenceSufficient"])
                and float(value["acceptedTopChoiceAccuracy"])
                >= VALIDATION_COHORT_PREFERENCE_FLOOR
            ...
            and all(
                ...
                and float(value["acceptedTopChoiceAccuracy"])
                >= VALIDATION_EVIDENCE_MODE_PREFERENCE_FLOOR
                for value in evidence_metrics.values()
            )
        )
        disagreement_core = {
            "schemaVersion": (
            ...
            "disagreementCount": len(disagreements),
            "carriedEquivalentCount": len(carried),
            "pendingAmbiguousCount": len(pending),
            "carriedEquivalentVerdicts": carried,
        ...
        report_core = {
            ...
            "correctionReportDigest": correction_report_digest,
            "sourceAdjudicationDigest": source_adjudication_digest,
            "sourceDisagreementDigest": source_disagreement_digest,
            "truthLineCount": len(truth_line_receipts),
            "truthLineReceipts": truth_line_receipts,
            "decisionCount": len(ranking_records),
            "decisionDigest": _sha256_json(ranking_records),
            "metrics": overall,
            "cohortMetrics": cohort_metrics,
            ...
            "disagreementDigest": disagreement_digest,
            "disagreementCount": len(disagreements),
            "carriedEquivalentDisagreementCount": len(carried),
            "pendingAmbiguousDisagreementCount": len(pending),
            "thresholds": {
                "overallAcceptedTopChoiceAccuracy": {
                    "comparison": "strictly_greater_than",
                    "value": VALIDATION_OVERALL_PREFERENCE_FLOOR,
                },
                "topThreeCoverage": (
                ...
                "cohortAcceptedTopChoiceAccuracy": (
                    VALIDATION_COHORT_PREFERENCE_FLOOR
                ),
                "evidenceModeAcceptedTopChoiceAccuracy": (
                    VALIDATION_EVIDENCE_MODE_PREFERENCE_FLOOR
                ),
                "mechanicalAccuracy": 1.0,
            },
            ...
            "gate": {
                ...
                "sealedTestAllowedAfterRulesFreeze": gate_passed,
                "reasons": [
                    *(
                        []
                        if not pending
                        else ["new_preference_ambiguities_need_review"]
                    ),
                    *(
                        []
                        if float(overall["acceptedTopChoiceAccuracy"])
                        > VALIDATION_OVERALL_PREFERENCE_FLOOR
                        else ["overall_preference_threshold_not_met"]
                    ),
                ],
            },
            "lineage": {
                ...
                "correctionRepositoryHead": correction_lineage.get(
                    "repositoryHead"
                ),
                "correctionCodeFileSha256": correction_lineage.get(
                    "correctionCodeFileSha256"
                ),
            },
            "evaluationCodeFileDigests": _rules_code_file_digests(
        ...
        report_path = (
            evaluation_dir
            / f"corrected-canonical-score-{report_digest}.json"
        )
        _write_json(report_path, report)
        _make_private(report_path)
        if gate_passed:
    ...
    def adjudicate_corrected_canonical_validation(
        self,
        model_id: str,
        *,
        score_report_digest: str,
        ...
        """Finalize corrected canonical validation without training from it.

        The corrected score may contain more than one authoritative cohort, so
        this operation consumes one immutable review submission per cohort. It
        accepts only the exact pending disagreements from the score report,
        combines them with byte-equivalent prior verdicts already carried by
        that report, and recomputes every fixed gate from strict counts.
        """

        if not re.fullmatch(r"[0-9a-f]{64}", score_report_digest):
            raise TrainingWorkflowError(
                "Corrected canonical adjudication needs an exact score digest."
            )
        normalized_submission_ids = tuple(
            str(value).strip() for value in submission_ids
        )
        if (
            not normalized_submission_ids
            or len(set(normalized_submission_ids))
            != len(normalized_submission_ids)
            or any(
                not re.fullmatch(
                    r"validation-disagreement-submission-[0-9a-f]{20}",
                    value,
                )
                for value in normalized_submission_ids
            )
        ):
            raise TrainingWorkflowError(
                "Corrected canonical adjudication needs unique exact submission IDs."
            )
        ...
        if not model_path.exists():
            raise TrainingWorkflowError(
                "The corrected canonical challenger artifact is missing."
            )
        model_sha256 = _sha256_bytes(model_path.read_bytes())
        if model_sha256 != str(model_meta.get("artifactSha256") or ""):
            raise TrainingWorkflowError(
        ...
        if not score_path.exists():
            raise TrainingWorkflowError(
                "The exact corrected canonical score report is missing."
            )
        ...
        if (
            ...
            or (
                score.get("noTrainingContract") or {}
            ).get("validationGroundTruthMayTrain") is not False
            ...
            raise TrainingWorkflowError(
                "Corrected canonical score lineage or no-training contract failed."
            )
        ...
        carried = [
            deepcopy(value)
            for value in disagreement.get("carriedEquivalentVerdicts") or ()
            if isinstance(value, Mapping)
        ]
        pending = [
            deepcopy(value)
            for value in disagreement.get("pendingAmbiguousDisagreements") or ()
            if isinstance(value, Mapping)
        ]
        carried_ids = {
            str(value.get("decisionId") or "") for value in carried
        }
        pending_ids = {
            str(value.get("decisionId") or "") for value in pending
        }
        if (
            ...
            or _sha256_json(disagreement_core) != disagreement_digest
            or int(disagreement.get("carriedEquivalentCount") or 0)
            != len(carried)
            or int(disagreement.get("pendingAmbiguousCount") or 0)
            != len(pending)
            or int(disagreement.get("disagreementCount") or 0)
            != len(carried) + len(pending)
            or int(score.get("disagreementCount") or 0)
            != len(carried) + len(pending)
            or len(carried_ids) != len(carried)
            or len(pending_ids) != len(pending)
            or "" in carried_ids
            or "" in pending_ids
            or carried_ids & pending_ids
            ...
            raise TrainingWorkflowError(
                "Corrected canonical disagreement lineage failed."
            )
        ...
        for item in pending:
            ...
            if (
                not decision_id
                or decision_id in pending_by_id
                or batch_id not in authoritative_ids
            ):
                raise TrainingWorkflowError(
                    "Corrected canonical disagreement identity is invalid."
                )
            pending_by_id[decision_id] = item
            pending_by_batch.setdefault(batch_id, set()).add(decision_id)
        if (
            not pending_by_batch
            or not set(pending_by_batch).issubset(authoritative_ids)
        ):
            raise TrainingWorkflowError(
                "Corrected canonical review contains no valid pending cohort."
            )
        if len(normalized_submission_ids) != len(pending_by_batch):
            raise TrainingWorkflowError(
                "Corrected canonical adjudication needs one submission per cohort."
            )
        ...
        for submission_id in normalized_submission_ids:
            ...
            for batch_id in sorted(pending_by_batch):
                review_dir = (
                    self._batch_dir(batch_id)
                    / "extraction"
                    / "validation"
                ...
                metadata_path = review_dir / f"{submission_id}.json"
                submission_path = review_dir / f"{submission_id}.jsonl"
                if metadata_path.exists() or submission_path.exists():
                    matches.append(
                        (batch_id, metadata_path, submission_path)
                    )
            if len(matches) != 1:
                raise TrainingWorkflowError(
                    "A corrected canonical submission is missing or ambiguous."
                )
            batch_id, metadata_path, submission_path = matches[0]
            if (
                batch_id in consumed_batches
                or not metadata_path.exists()
                or not submission_path.exists()
            ):
                raise TrainingWorkflowError(
                    "Corrected canonical submissions do not map one-to-one to cohorts."
                )
            ...
            packet_path = (
                ...
                / f"packet-{packet_digest}.json"
            )
            if not packet_path.exists():
                raise TrainingWorkflowError(
                    "The corrected canonical adjudication packet is missing."
                )
            packet = _read_json(packet_path)
            packet_core = {
                key: value
            ...
            if (
                ...
                or packet.get("modelId") != model_id
                or packet.get("modelArtifactSha256") != model_sha256
                or packet.get("correctedCanonicalScoreReportDigest")
                != score_report_digest
                or packet.get("disagreementReportDigest")
                != disagreement_digest
                or packet.get("packetDigest") != packet_digest
                or _sha256_json(packet_core) != packet_digest
                or expected_ids != pending_by_batch[batch_id]
                or packet.get("trainingEligible") is not False
                or packet.get("validationGroundTruthMayTrain") is not False
                or packet.get("sealedTestAccessed") is not False
            ):
                raise TrainingWorkflowError(
                    "Corrected canonical packet lineage or privacy failed."
                )
            rows = _read_jsonl(submission_path)
            submission_digest = _sha256_json(
                {
            ...
            if (
                ...
                raise TrainingWorkflowError(
                    "Corrected canonical submission lineage or privacy failed."
                )
            row_ids: set[str] = set()
            for row in rows:
                decision_id = str(row.get("decisionId") or "")
                ...
                if (
                    ...
                    raise TrainingWorkflowError(
                        "Corrected canonical adjudication contains an unknown decision."
                    )
                row_ids.add(decision_id)
                new_statuses[decision_id] = status
            if row_ids != expected_ids:
                raise TrainingWorkflowError(
                    "Corrected canonical adjudication is incomplete."
                )
        ...
        if consumed_batches != set(pending_by_batch):
            raise TrainingWorkflowError(
                "Corrected canonical adjudication did not cover every cohort."
            )
        ...
        for item in carried:
            ...
            if (
                decision_id not in carried_ids
                or decision_id in statuses
                or status
                not in {
                    "source_preferred",
                ...
                raise TrainingWorkflowError(
                    "A carried corrected verdict lost exact lineage."
                )
            statuses[decision_id] = status
        for decision_id, status in new_statuses.items():
            if decision_id in statuses:
                raise TrainingWorkflowError(
                    "A corrected canonical verdict was reviewed twice."
                )
            statuses[decision_id] = status
        if set(statuses) != carried_ids | pending_ids:
            raise TrainingWorkflowError(
                "Corrected canonical adjudication did not resolve every disagreement."
            )

        accepted_statuses = {"challenger_valid", "both_valid"}

        def accepted_metrics(
            strict_metrics: Mapping[str, Any],
            new_disagreement_ids: set[str],
        ) -> dict[str, Any]:
            metrics = deepcopy(dict(strict_metrics))
            decision_count = int(metrics.get("decisionCount") or 0)
            strict_correct = int(
                metrics.get("topChoiceCorrectCount")
                or metrics.get("strictSourceTopChoiceCorrectCount")
                or 0
            )
            carried_accepted = int(
                metrics.get("humanAcceptedEquivalentAlternativeCount")
                or metrics.get("humanAcceptedAlternativeCount")
                or 0
            )
            new_accepted = sum(
                statuses.get(decision_id) in accepted_statuses
                for decision_id in new_disagreement_ids
            )
            unresolved = sum(
                statuses.get(decision_id) == "feedback"
                for decision_id in new_disagreement_ids
            )
            metrics.update(
                {
                    "strictSourceTopChoiceCorrectCount": strict_correct,
                    "strictSourceTopChoiceAccuracy": (
                        strict_correct / decision_count
                        if decision_count
                        else 0.0
                    ),
                    "carriedHumanAcceptedAlternativeCount": (
                        carried_accepted
                    ),
                    "newHumanAcceptedAlternativeCount": new_accepted,
                    "humanAcceptedAlternativeCount": (
                        carried_accepted + new_accepted
                    ),
                    "newHumanRejectedAlternativeCount": sum(
                        statuses.get(decision_id) == "source_preferred"
                        for decision_id in new_disagreement_ids
                    ),
                    "humanUnresolvedAlternativeCount": unresolved,
                    "acceptedTopChoiceCorrectCount": (
        ...
        overall = accepted_metrics(
            score.get("metrics") or {},
            pending_ids,
        )
        cohort_metrics: dict[str, dict[str, Any]] = {}
        for batch_id, strict_metrics in (
            score.get("cohortMetrics") or {}
        ).items():
            if not isinstance(strict_metrics, Mapping):
                raise TrainingWorkflowError(
                    "Corrected canonical cohort metrics are malformed."
                )
            cohort_ids = {
                decision_id
                for decision_id, item in pending_by_id.items()
                if str(item.get("batchId") or "") == batch_id
            }
            cohort_metrics[str(batch_id)] = accepted_metrics(
                strict_metrics,
                cohort_ids,
            )
        evidence_metrics: dict[str, dict[str, Any]] = {}
        for evidence_tag, strict_metrics in (
            score.get("evidenceModeMetrics") or {}
        ).items():
            if not isinstance(strict_metrics, Mapping):
                raise TrainingWorkflowError(
                    "Corrected canonical evidence metrics are malformed."
                )
            evidence_ids = {
                decision_id
                for decision_id, item in pending_by_id.items()
                if str(evidence_tag) in (item.get("categoryTags") or ())
            }
            evidence_metrics[str(evidence_tag)] = accepted_metrics(
                strict_metrics,
                evidence_ids,
            )
        ...
        cohort_gate = bool(
            set(cohort_metrics) == set(authoritative_ids)
            and all(
                bool(metrics.get("evidenceSufficient"))
                and float(metrics.get("acceptedTopChoiceAccuracy") or 0.0)
                >= VALIDATION_COHORT_PREFERENCE_FLOOR
                for metrics in cohort_metrics.values()
            )
        )
        ...
        evidence_gate = bool(
            set(evidence_metrics) == required_evidence_tags
            and all(
                bool(metrics.get("evidenceSufficient"))
                and float(metrics.get("acceptedTopChoiceAccuracy") or 0.0)
                >= VALIDATION_EVIDENCE_MODE_PREFERENCE_FLOOR
        ...
        gate_passed = bool(
            unresolved_count == 0
            and float(overall.get("acceptedTopChoiceAccuracy") or 0.0)
            > VALIDATION_OVERALL_PREFERENCE_FLOOR
            and float(overall.get("topThreeCoverage") or 0.0)
            >= VALIDATION_TOP_THREE_COVERAGE_FLOOR
            and float(
                overall.get("approvedSourceMechanicalAccuracy") or 0.0
            )
            == 1.0
            and float(
                overall.get("predictedTopMechanicalAccuracy") or 0.0
            )
            == 1.0
            and cohort_gate
            and evidence_gate
        ...
        reasons = [
            *(
                ["human_feedback_requires_correction"]
                if unresolved_count
                else []
            ),
            *(
                ["overall_preference_threshold_not_met"]
                if float(
                    overall.get("acceptedTopChoiceAccuracy") or 0.0
                )
                <= VALIDATION_OVERALL_PREFERENCE_FLOOR
                else []
            ),
            *(
                ["top_three_threshold_not_met"]
                if float(overall.get("topThreeCoverage") or 0.0)
                < VALIDATION_TOP_THREE_COVERAGE_FLOOR
                else []
            ),
            *(
                ["mechanical_validity_threshold_not_met"]
                if (
                    float(
                        overall.get("approvedSourceMechanicalAccuracy")
                        or 0.0
                    )
                    != 1.0
                    or float(
                        overall.get("predictedTopMechanicalAccuracy")
                        or 0.0
                    )
                    != 1.0
                )
            ...
            *([] if cohort_gate else ["cohort_threshold_not_met"]),
            *(
                []
                if evidence_gate
                else ["evidence_mode_threshold_not_met"]
            ),
        ]
        report_core = {
            "schemaVersion": (
            ...
            "sourceScoreReportDigest": score_report_digest,
            "sourceDisagreementReportDigest": disagreement_digest,
            "sourceCorrectionReportDigest": score.get(
                "correctionReportDigest"
            ),
            "submissionReceipts": sorted(
                submission_receipts,
                key=lambda value: str(value["batchId"]),
            ),
            "carriedEquivalentVerdicts": carried,
            "newVerdictCounts": {
                status: sum(value == status for value in new_statuses.values())
                for status in sorted(set(new_statuses.values()))
            },
            "allDisagreementsAdjudicated": True,
            "metrics": overall,
            "cohortMetrics": cohort_metrics,
            "evidenceModeMetrics": evidence_metrics,
            "gate": {
        ...
        report_path = (
            evaluation_dir
            / f"corrected-canonical-adjudication-{report_digest}.json"
        )
        _write_json(report_path, report)
        _make_private(report_path)
        if gate_passed:
    ...
    def freeze_rules(self, model_id: str) -> dict[str, Any]:
        ...
        from steel_guitar_rag.amazing_tablature_decisions import DECISION_DERIVATION_VERSION
        from steel_guitar_rag.amazing_tablature_extraction import (
