...
- Latest backend movement-card commit: `254752a feat: add parameterized e9 movement cards`. This Lane 05 backend commit adds deterministic, original E9 movement examples for major-key `I-IV`, `I-V`, and `I-IV-V-I` prompts, including `Show me a G to C move.`, `Show me a G to D move.`, `Show me a 1 to 4 move in G.`, `Show me a 1 to 5 move in G.`, `Show me a 1 4 5 1 move in G.`, and `How do I connect no-pedals to A+B positions?`. Movement answers return direct prose, a validated `tab_example`, matching tab-derived fretboard payloads, `sources: []`, and `warnings: []`. Static prompts such as `Show me a G major grip.`, `Where is G on E9?`, and explicit `5-7-8` G grip prompts remain fretboard-first with no tab by default; Fender Steel King settings remain source-backed with no fretboard/tab payload. Focused and full checks passed: `py_compile` for changed backend modules, `tests/test_tab_engine.py`, focused and full `tests/test_api_search.py`, API/eval suites, full `.venv/bin/python -m pytest` with `878 passed`, and `git diff --check`. Local browser smoke passed at `http://127.0.0.1:8785/ui/steel-guitar-rag-mock.html?access=beta_user&v=parameterized-movement-cards-local-20260630`. Protected-preview restart/smoke is still pending; after restart, use `https://app.steelguitarrag.com/ui/steel-guitar-rag-mock.html?v=parameterized-movement-cards-254752a`.
