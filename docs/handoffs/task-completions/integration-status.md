...
- Latest Progression Guide simple-song routing fix: **PASS local browser smoke; protected-preview restart/smoke pending**. Implementation commit: `bf26e17 fix: route simple song progression prompts`. Lane 05 fixed the protected-smoke blocker where `How do I move through a simple song progression in G?` fell through to the generic specificity fallback and showed an empty source shell. The prompt now routes to a deterministic, source-free G I-IV-V-I Progression Guide with matching fretboard payload, no tab, no source cards, no empty source shell, and no `[object Object]`. Focused checks passed: `py_compile` for `pocketsteel/progression_guide.py` and `pocketsteel/api.py`; `tests/test_progression_guide.py` with 7 passed; focused `tests/test_api_search.py` progression/tab/static/source regressions with 29 passed; full `tests/test_api_search.py` with 290 passed; answer eval tests with 64 passed; full `.venv/bin/python -m pytest` with 890 passed; and `git diff --check`. Local browser smoke passed at `http://127.0.0.1:8786/ui/steel-guitar-rag-mock.html?access=beta_user&v=progression-guide-simple-song-fix-local` for the failed prompt plus explicit progression, movement, static grip, 5-7-8, and Steel King regressions. Lane 12 should restart/verify protected preview and smoke `https://app.steelguitarrag.com/ui/steel-guitar-rag-mock.html?v=progression-guide-simple-song-fix` before user smoke.
