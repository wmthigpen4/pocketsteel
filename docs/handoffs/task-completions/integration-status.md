...
- Latest deterministic Progression Guide v0 status: **PASS local API/browser smoke; protected-preview restart/smoke pending**. Implementation commit: `75fc370 feat: add deterministic progression guide`. Lane 05 added a deterministic, source-free `progression_guide` response contract for beginner E9 progression-route prompts, plus matching fretboard payloads and answer-page rendering. Covered prompts include `Show me a C F G C progression on E9.`, `How do I play I IV V I in C on pedal steel?`, `Give me a beginner route through G C D G.`, and `Show me C Am Em F Dm G7 C as a pedal steel progression.` Movement prompts such as `Show me a G to C move.` still use deterministic tab plus fretboard, while static prompts such as `Show me a G major grip.` remain fretboard-first with no tab. Full-song tab/transcription guardrails still return clean source-free refusals. Checks passed: `py_compile` for changed backend files, `node --check` for answer/fretboard JS, focused progression/API/eval/frontend/same-origin tests, full `.venv/bin/python -m pytest` with `888 passed`, and `git diff --check`. Local same-origin browser smoke passed at `http://127.0.0.1:8786/ui/steel-guitar-rag-mock.html?access=beta_user&v=progression-guide-v0-local`. Protected preview was not restarted in this Lane 05 run; Lane 12 should restart/verify runtime and smoke the direct app URL with a cache-bust such as `https://app.steelguitarrag.com/ui/steel-guitar-rag-mock.html?v=progression-guide-v0-75fc370`.
