...
- Protected-preview status for Explorer legitimate grip vocabulary: **WARN / authenticated protected static-browser behavior verified, strict clean-runtime certification blocked**. Protected URL `https://app.steelguitarrag.com/ui/e9-fretboard-explorer.html?v=legitimate-grip-vocabulary-c30f287` loaded after Cloudflare Access authentication and showed the legitimate grip-vocabulary behavior from `c30f287`: vocabulary controls for Core, Extended, Song/tab vocabulary, E-lower pockets, Two-string, and All legitimate; Voicing Identifier accepted 4-6-10 A+B, 3-5-9 open, 5-6-7 B, 6-7-10 A+B, 3-5-8 B+C, and 5-7-8 E-lower with tier/explanation/watch-out copy; Chord / Voicing Finder surfaced broader non-core candidates, Fmaj7 all-practical candidates, F7 candidates without major-7/dominant-7 mislabeling, and the 5-7-8 E-lower G pocket when E-lower vocabulary and lever scope were selected; path/default view remained readable; no `[object Object]`; no relevant browser console errors. Root `https://app.steelguitarrag.com/?v=legitimate-grip-vocabulary-c30f287` redirected to `/ui/steel-guitar-rag-mock.html` and dropped the query string. Strict user-smoke readiness remains blocked because local `/api/version` still reports stale runtime SHA `a6abc61` and the non-interactive documented LaunchDaemon restart failed because `sudo` requires a terminal password. The worktree also has dirty runtime-affecting Explorer/test files (`ui/e9-fretboard-explorer.js`, `tests/test_frontend_answer_ui.py`), so this smoke verifies protected served working-tree static behavior, not a clean committed-only runtime.
