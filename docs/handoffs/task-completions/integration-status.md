...
- Latest Progression Guide simple-song routing fix: **PASS protected-preview browser smoke at runtime `15a9c15`**. Implementation commit: `bf26e17 fix: route simple song progression prompts`; docs refresh commit: `15a9c15 docs: refresh integration status for simple progression fix`. Lane 12 refreshed the LaunchDaemon-supervised protected-preview runtime from stale `8c2536c` to `15a9c15`; `/api/version` now reports `15a9c15` with `auth_provider=cloudflare_access` and `retrieval_mode=hybrid_private_first`. Authenticated browser smoke passed at `https://app.steelguitarrag.com/ui/steel-guitar-rag-mock.html?v=progression-guide-simple-song-fix`. The previous blocker prompt `How do I move through a simple song progression in G?` now renders deterministic, source-free G I-IV-V-I Progression Guide v0 with matching fretboard payload, no tab, no source cards, no empty source shell, and no specificity fallback. Regression prompts passed: `Show me a 1 4 5 1 progression in G.`, `Show me a G C D G progression route.`, `Show me a G to C move.`, `Show me a G major grip.`, `Where is G on E9?`, `Show me a 5-7-8 G grip.`, and `What are good Fender Steel King settings?`. Root `https://app.steelguitarrag.com/?v=progression-guide-simple-song-fix` redirects to `/ui/steel-guitar-rag-mock.html` and drops the query string, so direct `/ui/...?...` remains the user-smoke URL. No relevant console errors or `[object Object]` were observed. User smoke may continue at the direct protected URL above.
