...
Serve the repository locally and open `/ui/chord-reader-proof/` to inspect three public-domain songs against 49 hand-authored bars. The synchronized player switches among a licensed human performance of “Amazing Grace” and app-owned deterministic performances of “When the Saints Go Marching In” and “Oh! Susanna.” It shows the chart, current v2 output, v8 output, and the no-chord safety overlay for every bar; any bar can be clicked to hear the underlying audio.

The v8 candidate separates root, quality, and boundary evidence. Three multiband chord experts feed a harmonic-CQT root guide. Two quality-objective Transformers contribute only when their mode or extension predictions agree above frozen confidence thresholds. Old and NRG-CP-trained boundary heads are averaged. A 14-feature whole-track texture gate uses the conservative v4 path for high-confidence sparse audio and the expanded path for mixtures.

Across the player suite, v8 identifies 46 of 49 musical bars versus 43 of 49 for v2. Duration-weighted major/minor WCSR is 90.8% for v8 versus 85.9% for v2. The page keeps the real-song limitation visible: on “Amazing Grace,” v8 finds 14/16 dominant bars versus 13/16 for v2, while continuous major/minor WCSR is 79.94% for v8 and 79.99% for v2 (v8 is lower by roughly 0.05 point). No single-song regression is hidden behind the suite average.

The tracked multi-corpus table contains 366 recordings and 3.71 hours across GuitarSet, AAM, Winterreise, IDMT, NRG-CP, and BabySlakh. Those results are an iterative held-out audit because later architecture work followed earlier test inspection. A separate 500-composition NRG-CP confirmation set was sampled after v8 froze, excludes every prior composition, has zero training weight, and was scored once. On that pristine 132-minute confirmation, v8 versus v4 changes root WCSR by +0.38 point, major/minor by +2.80, detailed WCSR by +6.40, boundary F1 by +3.06, and sequence edit rate by −9.64. Absolute v8 scores are 81.69% root, 81.11% major/minor, 67.68% detailed, and 76.64% boundary F1. The 98% objective has not been met.

NRG-CP is symbolic and deterministically rendered, so its strong delta is not treated as equivalent to human-recorded music. BabySlakh is a separate 81-minute audit of professional sampled-instrument mixtures with aligned score-derived references; those references are not represented as human chord annotations. Travis songs remain unused.
